...
class FakeTrackingRepo:
    ...
    def create(self, carrier_id, shipment_id, lat, lon):
        t = SimpleNamespace(
            id=self._next_id,
    ...
    def latest_for_shipment(self, shipment_id):
        for t in reversed(self._traces):
            if t.shipment_id == shipment_id:
                return t
        return None

    def history_for_shipment(self, shipment_id, limit=500):
...
class FakeCarrierRepo:
    ...
    def update(self, carrier_id, **kwargs):
...
class FakeShipmentRepo:
    ...
    def list_active_by_carrier(self, carrier_id):
        return [
...
def test_publish_position_no_active_shipments_returns_zero():
    carrier = make_carrier(id=1)
    svc, repo = make_service(carriers=[carrier], shipments=[])
    count = svc.publish_position(carrier_id=1, lat=-34.6037, lon=-58.3816)
    assert count == 0


def test_publish_position_no_active_shipments_still_records_trace():
    """Even with no active shipments a positional trace is recorded."""
    carrier = make_carrier(id=1)
    svc, repo = make_service(carriers=[carrier], shipments=[])
    svc.publish_position(carrier_id=1, lat=-34.6037, lon=-58.3816)
    assert len(repo._traces) == 1
    assert repo._traces[0].shipment_id is None


def test_publish_position_with_active_shipments_returns_count():
    carrier = make_carrier(id=1)
    ...
    count = svc.publish_position(carrier_id=1, lat=-34.6, lon=-58.4)
    assert count == 2
    assert len(repo._traces) == 2


def test_publish_position_updates_carrier_location():
    carrier = make_carrier(id=1)
    svc, _ = make_service(carriers=[carrier])
    svc.publish_position(carrier_id=1, lat=-34.9, lon=-57.9)
    assert carrier.current_lat == -34.9
    assert carrier.current_lon == -57.9


def test_publish_position_pending_shipment_not_traced():
...
def test_shipment_location_not_found_raises_value_error():
    svc, _ = make_service()
    with pytest.raises(ValueError, match="Shipment not found"):
        svc.shipment_location(shipment_id=999, requester_user_id=1)


def test_shipment_location_pending_status_returns_none():
    ...
    result = svc.shipment_location(shipment_id=1, requester_user_id=5)
    assert result is None


def test_shipment_location_delivered_status_returns_none():
...
def test_shipment_location_unauthorized_raises_permission_error():
    ...
    with pytest.raises(PermissionError):
        svc.shipment_location(shipment_id=1, requester_user_id=99)


def test_shipment_location_authorized_as_client_returns_trace():
    carrier = make_carrier(id=1, user_id=10)
    shipment = make_shipment(id=1, carrier_id=1, client_id=5,
                             status=ShipmentStatus.ASSIGNED)
...
def test_shipment_location_authorized_as_carrier_user():
    carrier = make_carrier(id=1, user_id=10)
    shipment = make_shipment(id=1, carrier_id=1, client_id=5,
    ...
    svc, repo = make_service(carriers=[carrier], shipments=[shipment])
    repo._traces.append(trace)
    result = svc.shipment_location(shipment_id=1, requester_user_id=10)
    assert result is not None
    assert result.lat == -34.7


def test_shipment_location_no_traces_returns_none():
    carrier = make_carrier(id=1, user_id=10)
    shipment = make_shipment(id=1, carrier_id=1, client_id=5,
                             status=ShipmentStatus.ASSIGNED)
    svc, _ = make_service(carriers=[carrier], shipments=[shipment])
...
def test_shipment_history_returns_empty_list_when_no_traces():
    svc, _ = make_service()
    history = svc.shipment_history(shipment_id=42)
    assert history == []


def test_shipment_history_returns_traces_in_order():
    ...
    history = svc.shipment_history(shipment_id=7)
