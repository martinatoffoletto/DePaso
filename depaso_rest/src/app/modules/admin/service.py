...
class AdminService:
    ...
    def dashboard(self) -> dict:
        ...
        delivered = self._count(Shipment, Shipment.status == ShipmentStatus.DELIVERED)
        cancelled = self._count(Shipment, Shipment.status == ShipmentStatus.CANCELLED)
        closed = delivered + cancelled
        co2_total = (
            self.db.query(func.coalesce(func.sum(Shipment.co2_savings_kg), 0.0))
            .filter(Shipment.status == ShipmentStatus.DELIVERED)
            .scalar()
        )
        return {
            "total_users": self._count(User),
            "total_carriers": self._count(Carrier),
            "carriers_pending_verification": self._count(
                Carrier, Carrier.is_verified == False, Carrier.is_active == True  # noqa: E712
            ),
            "shipments_total": self._count(Shipment),
            "shipments_active": self._count(Shipment, Shipment.status.in_(ACTIVE_STATUSES)),
            "shipments_delivered": delivered,
            "shipments_pending": self._count(Shipment, Shipment.status == ShipmentStatus.PENDING),
            "total_co2_saved_kg": round(float(co2_total), 3),
            "matching_success_rate": round(delivered / closed, 4) if closed else 0.0,
        }

    def pending_carriers(self) -> list[Carrier]:
        """Carriers awaiting verification (RF-USR-07)."""
        return (
            self.db.query(Carrier)
            .filter(Carrier.is_verified == False, Carrier.is_active == True)  # noqa: E712
            .order_by(Carrier.created_at)
            .all()
        )

    def moderate_carrier(self, carrier_id: int, action: str) -> Carrier | None:
        """Verify / suspend / reactivate a carrier. Returns None if not found;
        raises ValueError for an invalid action."""
        if action not in VALID_MODERATION_ACTIONS:
            raise ValueError("action must be verify | suspend | reactivate")
    ...
    def system_status(self, classifier) -> dict:
    ...
    def recent_activity(self, limit: int = 20) -> dict:
        ...
        return {"classifications": classifications, "events": events}
