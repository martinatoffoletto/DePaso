...
const hhmm = (iso: string) => parseApiDate(iso).toLocaleTimeString("es-AR", { hour: "2-digit", minute: "2-digit" });
const dayLabel = (iso: string) => parseApiDate(iso).toLocaleDateString("es-AR", { weekday: "short", day: "numeric", month: "short" });

/** Published trip card (offline state): trayecto colaborativo o turno en zona. */
    ...
    <View className="bg-card border border-border rounded-2xl px-[14px] py-3">
      <View className="flex-row items-center gap-2 mb-[10px]">
        <MaterialCommunityIcons
          name={turno ? "truck-cargo-container" : route.recurrence_days ? "repeat" : "calendar-star"}
          size={14}
          color={T.emeraldDeep}
        />
        <Text className="text-[11px] tracking-[1px] text-inkMute font-bold uppercase">
          {turno ? "Turno en zona" : route.recurrence_days ? "Ruta habitual" : "Viaje especial"}
        </Text>
      </View>
      {turno ? (
      ...
      {route.recurrence_days ? (
        ...
        <Text className="text-[10px] tracking-[1px] text-inkMute uppercase mt-2">
          {dayLabel(route.window_start)}{" · "}{hhmm(route.window_start)} a {hhmm(route.window_end)}
        </Text>
      ) : (
        <Text className="text-[10px] tracking-[1px] text-inkMute uppercase mt-2">
          {dayLabel(route.window_start)}{" · "}{hhmm(route.window_start)}
        </Text>
      )}
    </View>
  );
